...
[ $music_object_type = "liked" ] && {
    rofi_out="$(echo -n $(md.py -S liked -s time_liked -o song -I "name - album - artist\n") |\
        rofi -async-pre-read 1 -dmenu -i -p "${music_object_type}s" -multi-select)"
} 
[ $music_object_type = "song" ] && {
    rofi_out="$(echo -n $(md.py -S all -s id -o song -I "name - album - artist\n") |\
        rofi -async-pre-read 1 -dmenu -i -p "${music_object_type}s" -multi-select)"
}
[ $music_object_type = "album" ] && {
    rofi_out="$(echo -n $(md.py -S all -s id -o album -I "name - artist\n") |\
        rofi -async-pre-read 1 -dmenu -i -p "${music_object_type}s" -multi-select)"
}
